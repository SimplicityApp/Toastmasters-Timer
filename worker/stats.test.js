...
describe('handleStats', () => {
  ...
  it('returns live numbers from the PostHog query result', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        new Response(JSON.stringify({ results: [[600, 60, 1600]] }), { status: 200 })
      )
    );

    const res = await handleStats(request, { POSTHOG_QUERY_API_KEY: 'phx_test' }, ctx);
    const body = await res.json();
    expect(body).toEqual({ timerUsers: 600, countries: 60, speechesTimed: 1600, fallback: false });
    // A successful answer is cacheable for hours; the fallback is not.
